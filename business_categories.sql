...
-- Create activities tracking function for admin dashboard
CREATE OR REPLACE FUNCTION log_business_category_activity()
RETURNS TRIGGER AS $$
BEGIN
    -- Insert a record into the activities table for admin dashboard
    ...
    VALUES (
        CASE 
            ...
            WHEN TG_OP = 'UPDATE' THEN 'Category Updated'
            WHEN TG_OP = 'DELETE' THEN 'Category Deleted'
        END,
        'admin',
        CASE 
            WHEN TG_OP = 'INSERT' THEN 'New business category "' || NEW.title || '" was created'
            WHEN TG_OP = 'UPDATE' THEN 'Business category "' || NEW.title || '" was updated'
            WHEN TG_OP = 'DELETE' THEN 'Business category "' || OLD.title || '" was deleted'
        END,
        auth.uid(),
        'business_categories',
        auth.uid() -- Use the authenticated user's UUID instead of trying to convert the category ID
    );
...
CREATE TRIGGER business_categories_activity_insert
AFTER INSERT ON business_categories
FOR EACH ROW EXECUTE FUNCTION log_business_category_activity();

CREATE TRIGGER business_categories_activity_update
AFTER UPDATE ON business_categories
FOR EACH ROW EXECUTE FUNCTION log_business_category_activity();

CREATE TRIGGER business_categories_activity_delete
AFTER DELETE ON business_categories
FOR EACH ROW EXECUTE FUNCTION log_business_category_activity();
